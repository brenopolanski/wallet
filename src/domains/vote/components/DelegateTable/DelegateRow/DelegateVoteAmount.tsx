...
export const DelegateVoteAmount = ({
	...
	const calculateNetAmount = (inputValue: number, isDisabled: boolean) => {
		...
		const isEqualToAmountVoted = amountVoted === inputValue && amount === 0;
		const isUnvoteOrZeroAmount = (isDisabled || inputValue === 0) && amountVoted === amount;
		const isLessThanAmountVoted = amountVoted > inputValue && inputValue !== 0 && amount <= amountVoted;
		...
		// on copy/paste
		const isEqualToAmountVotedAndHasAmount = amountVoted === inputValue && amountVoted !== amount;
		...
		const hasAmount = amountVoted === amount && inputValue > amountVoted;
		if (hasAmount) {
			netAmount -= amountVoted;
			return netAmount;
		}

		const isGreaterThanAmountVoted = amount > 0 && inputValue > amountVoted;
