...
import { getColor } from "domains/dashboard/components/PortfolioBreakdown/PortfolioBreakdown.helpers";
...
const AssetListItem: React.VFC<AssetListItemProperties> = ({ asset, index, exchangeCurrency, isCompact }) => {
	...
	return (
		<TableRow>
			<TableCell variant="start" innerClassName="space-x-3" isCompact>
				<div className={`w-1 h-5 rounded bg-theme-${color}`}></div>
...
const AssetList: React.VFC<AssetListProperties> = ({ data, exchangeCurrency }) => {
	...
	const renderTableRow = useCallback(
		(asset: AssetItem, index: number) => (
			<AssetListItem asset={asset} index={index} exchangeCurrency={exchangeCurrency} isCompact />
		),
		[exchangeCurrency],
	);
